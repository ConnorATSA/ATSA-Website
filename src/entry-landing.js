...
(function(){
  ...
  function Header() {
    ...
    return (
      <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(11,59,94,0.9)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.12)' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', padding: isMobile ? '12px 18px' : '14px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
          <a href="#/" onClick={(e) => { e.preventDefault(); window.ATSANav('#/'); }} style={{ display: 'flex' }}><img src={window.__resources?.logoReversed || "../../assets/logos/atsa-logo-reversed.svg"} alt="ATSA" style={{ height: isMobile ? 32 : 38 }} /></a>
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
            {!isMobile && <a href="#/challenge/app" style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: 14.5, color: 'rgba(255,255,255,0.85)', textDecoration: 'none', padding: '8px 10px' }}>Log in</a>}
            <Button variant="accent" iconRight="arrow-right" href="#/challenge/app">{isMobile ? 'Start' : 'Start the challenge'}</Button>
          </div>
        </div>
      </header>
    );
  ...
  function Hero() {
    ...
    return (
      <section style={{ position: 'relative', background: 'var(--atsa-navy)', color: '#fff', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 1120, margin: '0 auto', padding: isMobile ? '48px 20px 64px' : '72px 28px 96px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.1fr 0.9fr', gap: isMobile ? 30 : 48, alignItems: 'center' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: isMobile ? 38 : 60, lineHeight: 1.02, letterSpacing: '-0.02em', margin: '20px 0 0', textWrap: 'balance' }}>
              The 1% Better Challenge
            </h1>
            <p style={{ fontSize: isMobile ? 17 : 20, lineHeight: 1.55, color: 'rgba(255,255,255,0.82)', maxWidth: 500, margin: '20px 0 32px' }}>
  ...
  function How() {
    ...
    return (
      <section id="how" style={{ maxWidth: 1120, margin: '0 auto', padding: isMobile ? '44px 20px' : '64px 28px' }}>
        <div style={{ textAlign: 'center', maxWidth: 620, margin: '0 auto 44px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: isMobile ? 28 : 36, letterSpacing: '-0.02em', color: 'var(--atsa-navy)', margin: '0 0 12px', lineHeight: 1.15 }}>Consistency beats intensity</h2>
          <p style={{ fontSize: isMobile ? 16 : 17, color: 'var(--atsa-slate)', lineHeight: 1.6, margin: 0 }}>You don’t become a better teacher in one heroic leap — you become one by getting slightly better, on purpose, every day. A missed day isn’t failure; just pick it up tomorrow.</p>
        </div>
  ...
  function Themes() {
    ...
    return (
      <section style={{ background: 'var(--atsa-cloud)' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', padding: isMobile ? '44px 20px' : '64px 28px' }}>
          <div style={{ textAlign: 'center', maxWidth: 600, margin: '0 auto 40px' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: isMobile ? 28 : 36, letterSpacing: '-0.02em', color: 'var(--atsa-navy)', margin: 0, lineHeight: 1.15 }}>Ten themes · ten days each</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isNarrow ? 'repeat(2, 1fr)' : isMobile ? 'repeat(3, 1fr)' : 'repeat(5, 1fr)', gap: isMobile ? 12 : 14 }}>
            {C.themes.map((t) => (
              <div key={t.n} style={{ background: '#fff', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-subtle)' }}>
                ...
                <div style={{ padding: '14px 16px' }}>
                  <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: 15.5, color: 'var(--atsa-navy)', lineHeight: 1.25, marginBottom: 6, overflowWrap: 'anywhere' }}>{t.name}</div>
  ...
  function Preview() {
    ...
    return (
      <section style={{ maxWidth: 1120, margin: '0 auto', padding: isMobile ? '44px 20px' : '64px 28px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 48, alignItems: 'center' }}>
        <div>
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: isMobile ? 27 : 34, letterSpacing: '-0.02em', color: 'var(--atsa-navy)', margin: '0 0 16px', lineHeight: 1.15 }}>A daily check-in built for the pool deck</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[['check-circle', 'One or two taps', 'See today’s action, mark it done. That’s it.'], ['droplet', 'Forgiving streaks', 'Miss a day? No shame, no reset — just pick it up.'], ['award', 'Badges & milestones', 'Celebrate each theme, and 25, 50, 75, 100 days.'], ['book-open', 'A growth journal', 'Your reflections, saved across all 100 days.']].map(([icon, t, d]) => (
              <div key={t} style={{ display: 'flex', gap: 13, alignItems: 'flex-start' }}>
                ...
                <div style={{ minWidth: 0 }}><div style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: 16.5, color: 'var(--atsa-navy)' }}>{t}</div><div style={{ fontSize: 14, color: 'var(--atsa-slate)', lineHeight: 1.5 }}>{d}</div></div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', order: isMobile ? -1 : 0 }}>
          <Card padding="none" style={{ overflow: 'hidden', maxWidth: 320, width: '100%', boxShadow: 'var(--shadow-lg)', border: 'none' }}>
            <div style={{ background: sample.themeColor, color: '#fff', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 11 }}>
              <span style={{ display: 'inline-flex', width: 40, height: 40, borderRadius: 11, background: 'rgba(255,255,255,0.18)', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon name={sample.themeIcon} size={21} /></span>
              <div style={{ minWidth: 0 }}><div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', opacity: 0.85 }}>Theme 1 · {sample.themeName}</div><div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 18 }}>Day 1</div></div>
            </div>
  ...
  function CTA() {
    ...
    return (
      <section style={{ background: 'var(--atsa-ocean)', color: '#fff' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', padding: isMobile ? '48px 20px' : '64px 28px', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: isMobile ? 28 : 38, letterSpacing: '-0.02em', margin: '0 0 14px' }}>Ready to get 1% better?</h2>
          <p style={{ fontSize: isMobile ? 16 : 18, color: 'rgba(255,255,255,0.85)', margin: '0 0 28px' }}>Start today. 100 tiny changes, one day at a time.</p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button variant="inverse" size="lg" iconRight="arrow-right" href="#/challenge/app">Start the challenge</Button>
            <Button variant="inverse-secondary" size="lg" iconLeft="mail" onClick={() => window.ATSANav('#/')}>Bring it to your swim school</Button>
          </div>
        </div>
      </section>
    );
